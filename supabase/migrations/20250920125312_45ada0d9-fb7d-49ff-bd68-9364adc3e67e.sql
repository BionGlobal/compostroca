-- Remover unidades fictícias CWB002 e CWB003
DELETE FROM unidades WHERE codigo_unidade IN ('CWB002', 'CWB003');

-- Atualizar data_finalizacao dos lotes encerrados baseada na última manutenção
UPDATE lotes 
SET data_finalizacao = subquery.ultima_manutencao 
FROM (
  SELECT 
    l.id,
    MAX(ms.created_at) as ultima_manutencao
  FROM lotes l
  JOIN manejo_semanal ms ON ms.lote_id = l.id AND ms.deleted_at IS NULL
  WHERE l.status = 'encerrado' AND l.data_finalizacao IS NULL
  GROUP BY l.id
) as subquery
WHERE lotes.id = subquery.id;

-- Atualizar função buscar_lotes_finalizados para garantir que encontre lotes finalizados
