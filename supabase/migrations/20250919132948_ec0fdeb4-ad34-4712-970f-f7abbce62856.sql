...
-- 1) lotes
CREATE POLICY "Approved local/super admins can delete lotes"
ON public.lotes
FOR DELETE
USING (can_modify_data() AND has_unit_access(unidade));

-- 2) entregas  
CREATE POLICY "Approved local/super admins can delete entregas"
ON public.entregas
FOR DELETE
USING (
...
-- 3) manejo_semanal
CREATE POLICY "Approved local/super admins can delete manejo"
