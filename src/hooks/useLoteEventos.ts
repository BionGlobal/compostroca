...
export const useLoteEventos = () => {
  ...
  const fetchEventosByLote = async (loteId: string): Promise<LoteEvento[]> => {
    ...
    try {
      ...
      return (data || []).map(evento => ({
        ...
        tipo_evento: evento.tipo_evento as 'inicio' | 'manutencao' | 'finalizacao',
        ...
        fotos_compartilhadas: Array.isArray(evento.fotos_compartilhadas) 
          ? (evento.fotos_compartilhadas as string[])
          : [],
        dados_especificos: typeof evento.dados_especificos === 'object' && evento.dados_especificos !== null
      ...
      toast({
        title: 'Erro',
        description: 'Não foi possível carregar os eventos do lote',
        variant: 'destructive'
      });
      return [];
    } finally {
  ...
  const criarEventoManutencao = async (
    ...
    setLoading(true);
    try {
      // Calcular peso com decaimento de 3.66%
      const pesoDepois = Number((pesoAnterior * (1 - 0.0366)).toFixed(2));
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          etapa_numero: etapaNumer,
          peso_antes: pesoAnterior,
          peso_depois: pesoDepois,
          caixa_origem: etapaNumer,
          caixa_destino: etapaNumer + 1,
          latitude,
          longitude,
          ...
          dados_especificos: {
            peso_estimado: true,
            taxa_decaimento_aplicada: 0.0366,
            calculo: `${pesoAnterior} * (1 - 0.0366) = ${pesoDepois}`
          }
        })
        .select()
        .single();
      ...
      toast({
        title: 'Sucesso',
        description: `Manutenção da semana ${etapaNumer - 1} registrada com sucesso`
      });
